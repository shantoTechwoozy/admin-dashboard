...
import { VisaHistoryInterface } from "@/types/common/interfaces";
import PageTitle from "@/components/paymentRequests/PageTitle";


const VisaHistory = () => {
    const [sortField, setSortField] = useState<keyof VisaHistoryInterface>('booked_by');
    const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('asc');
    const [searchQuery, setSearchQuery] = useState<string>('');
    const [searchField, setSearchField] = useState<keyof VisaHistoryInterface>('booking_code');

    const handleFieldChange = (item: string) => {
        setSearchField(item as keyof VisaHistoryInterface);
    };
    const handleSort = (field: keyof VisaHistoryInterface) => {
        const order = sortField === field && sortOrder === 'asc' ? 'desc' : 'asc';
        setSortField(field);
        setSortOrder(order);
    };

    const filteredData = visaHistoryData
        .filter((item) =>
    ...
    return (
        <>
            ...
            <TableWrap>
                ...
                fieldLists={visaHistoryFields.map((element) => element.label)}
            /> */}
                <TableContainer>
                    <TableHeader
                        items={visaHistoryFields.map((element) => element.label)}
                    />
                    <TableBody
                        items={filteredData}
                    />
